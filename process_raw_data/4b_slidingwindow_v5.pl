...
use warnings;
use strict;
#This script is designed to work off the output of the previous script. It runs a sliding window summing dxy, Fst (num and denom) and heterozygosity expected. It also prints out the number of sites (including invariant) and variable sites (excluding invariant). The window size is taken from the first arguement used when calling the script. The stats file is taken from standard in.

#Usage cat statsfile.txt |  ./script_name.pl <window_size> > slidingwindow_out.txt
#Window variables
my $current_chr;
