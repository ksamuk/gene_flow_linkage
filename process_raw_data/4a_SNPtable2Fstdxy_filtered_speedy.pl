#!/usr/bin/perl
#This script takes a flat tab separated SNP table as input. It also takes a file that links the sample names to a population name (eg. sample1\tpop1). Finally it takes a file that describes the two groups Fst will be calculated against (eg. pop1\t1). The two groups are labelled "1" and "2" and can each contain multiple populations. Not all samples need to be analyzed, if a population is excluded from a group the samples will be ignored.
#For each locus it calculates sample sizes, the numerator and denominator of Weir and Cockerham Fst (useful for calculating multiple locus Fst), Weir and Cockerham Fst, Heterozyosity expected within each group and dxy. dxy is uninformative on an individual site basis and should be used in a sliding window context.
#Sites without the minimum sample size or minimum allele frequency are skipped (although invariant sites with the minimum sample size are printed).
#Sites with greater than 2 alleles are skipped.
use warnings;
use strict;
...
while (<STDIN>){
	...
	my @a = split (/\t/,$_);
  	if ($. == 1){
		...
		print "\tN1\tN2\tNTotal\tDxy\tFstNum\tFstDenom\tFst\tHexp1\tHexp2";
	}
	else{
		next if /^\s*$/;
		my %BC;
